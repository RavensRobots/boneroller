...
def start(update, context):
    ...
    context.bot.send_message(chat_id=update.effective_chat.id, text=interface.greet())
...
def d6(update, context):
    ...
    context.bot.send_message(chat_id=update.effective_chat.id, text=interface.roll_a_dice(6))
...
def pig(update, context):
    ...
    if chat.type == "group" or chat.type == "supergroup":
        text, keyboard = interface.get_game(chat, user, "pig")
        reply_markup = InlineKeyboardMarkup(keyboard)
